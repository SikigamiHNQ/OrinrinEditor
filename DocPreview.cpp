...
#pragma message ("全プレ書換時に、スクロールバーの位置覚えておいて、そこまでScrollさせる？")
	...
	ghPrevWnd = CreateWindowEx( WS_EX_TOOLWINDOW, DOC_PREVIEW_CLASS, TEXT("IE内核下进行预览"),
	...
	StringCchCopy( atBuffer, MAX_STRING, TEXT("全预览风格") );	gstTBInfo[0].iString = SendMessage( ghToolWnd, TB_ADDSTRING, 0, (LPARAM)atBuffer );
	...
	if( SUCCEEDED( AtlAxGetControl( ghIEwnd, &comPunkIE ) ) )
	{
		...
		{
			NotifyBalloonExist( TEXT("IE内核初始化失败"), TEXT("阿燐燐向您确认"), NIIF_ERROR );
...
HRESULT PreviewPageWrite( INT iViewPage )
{
	...
	if( !(gpcHtmlHdr) )
	{
		NotifyBalloonExist( TEXT("未能找到预览用的模板文件哦"), TEXT("阿燐燐向您确认"), NIIF_ERROR );
		return E_HANDLE;
	}
