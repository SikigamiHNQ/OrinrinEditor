...
HWND DockingTabCreate( HINSTANCE hInst, HWND hPrWnd, LPRECT pstRect )
{
	...
	stTcItem.pszText = TEXT("单行");	TabCtrl_InsertItem( hWorkWnd, 0, &stTcItem );
	stTcItem.pszText = TEXT("笔刷");	TabCtrl_InsertItem( hWorkWnd, 1, &stTcItem );
...
HRESULT DockingTabContextMenu( HWND hWnd, HWND hWndContext, LONG xPos, LONG yPos )
{
	...
	if( gbDockTmplView )	AppendMenu( hPopupMenu, MF_STRING, IDM_LINE_BRUSH_TMPL_VIEW, TEXT("隐藏模板") );
	else					AppendMenu( hPopupMenu, MF_STRING, IDM_LINE_BRUSH_TMPL_VIEW, TEXT("显示模板") );
...
UINT TemplateGridFluctuate( HWND hLvWnd, INT dFluct )
{
	...
	TRACE( TEXT("列增减[%u][%d]"), clmCount, dFluct );
