...
INT TreeProfileMake( HWND hWnd, LPTSTR ptProf )
{
	...
	else	//	開く指定がなかったら、こっちから指定する
	{
		...
		stOpenFile.lpstrFilter     = TEXT("索引文件(*.qor)\0*.qor\0全部文件(*.*)\0*.*\0\0");
//		stOpenFile.nFilterIndex    = 1;
	...
	//	stOpenFile.lpstrInitialDir = 
		stOpenFile.lpstrTitle      = TEXT("请指定文件、或者输入要新建的文件名呢");
		...
		case WM_COMMAND:
			...
			switch( id )
			{
				...
				case IDOK:
					...
					if( 1 >= count )
					{
						MessageBox( hDlg, TEXT("建立索引失败。\r\nAA库树状图生成失败。"), TEXT("阿燐燐向您确认"), MB_OK | MB_ICONERROR );
						return (INT_PTR)TRUE;
					}
...
HRESULT TreeLoadDirCheck( HWND hDlg, HWND hTvWnd )
{
	...
	{
		ShowWindow( hWorkWnd, SW_HIDE );
		MessageBox( hDlg, TEXT("没有选择任何文件哦。\r\n将无法使用文件哦。"), TEXT("阿燐燐向您确认"), MB_OK | MB_ICONERROR );
		return E_ABORT;
	}
...
//	HWND	hWndChild;
	...
	switch( message )
	{
		...
		case WM_INITDIALOG:
			...
			stLvColm.pszText = TEXT("文件名");	stLvColm.cx = 250;	stLvColm.iSubItem = 0;	ListView_InsertColumn( hWorkWnd, 0, &stLvColm );
...
HRESULT OpenProfileInitialise( HWND hWnd )
{
	...
	if( hWnd )	//	ロード
	{
		...
		AppendMenu( ghProfHisMenu, MF_STRING, IDM_OPEN_HIS_CLEAR, TEXT("清空历史记录") );
